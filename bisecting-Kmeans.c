/** @file group_1.c
 *  @brief bisecting Kmeans implementation
 *
 *  This contains the required implemented
 *  methods for Big Data Assigment 5
 *
...
/**Selecting two centers for 2-means on X and obtain two clusters 
 * with center C1, C2*/
void apply_2_means(int dim, int i0, int im, double *data, int k, 
    ...
    if(LOG) printf("Second point is %d \n",cluster_center_points[1] );
...
double getSSEOfCluster(int cluster_index, int dim, double *data, int k, 
                    int *cluster_start, int *cluster_size,
                   double *cluster_centroid ) {
    // code 
    double cost = 0;
    for(int j = cluster_start[cluster_index]; 
        j < cluster_start[cluster_index] +cluster_size[cluster_index]; j++){
        
        cost += euclidean_distance(
                &cluster_centroid[cluster_index],
                &data[j*dim],
                dim);
 ...
 */
int initial_centers_bkm(int dim, int i0, int im, double *data, int k, 
                    double *cluster_centroid) {
    
    int cluster_start_temp[2];
    int cluster_size_temp[2];
    int cluster_start[k];
    int cluster_size[k];
    
    double cluster_sse[k];
    double cluster_radius[2];
    
    double cluster_centroid_temp[2*dim];
    //short cluster_assign[im-i0+1];
    short *cluster_assign = (short*) malloc((im-i0+1) * sizeof(double));
    // defaulting all the values
    memset(cluster_start,-1, sizeof(cluster_start));
    memset(cluster_size,-1, sizeof(cluster_size));
    memset(cluster_start_temp,-1, sizeof(cluster_start));
    memset(cluster_size_temp,-1, sizeof(cluster_size));
    memset(cluster_radius,0, sizeof(cluster_radius));
    memset(cluster_centroid_temp,0, sizeof(cluster_centroid_temp));    
    memset(cluster_assign,-1, sizeof(cluster_assign));
    memset(cluster_sse,-1, sizeof(cluster_sse));
    
    apply_2_means(dim, i0,im,data,2,cluster_start_temp,
                     cluster_size_temp,cluster_radius,cluster_centroid_temp,
                                        cluster_assign);
    
    for(int i = 0, t =0; i < 2*dim; i++,t++){
        cluster_centroid[t] = cluster_centroid_temp[i];
    }
    
    cluster_start[0] = cluster_start_temp[0];
    cluster_size[0] = cluster_size_temp[0];
    cluster_start[1] = cluster_start_temp[1];
    cluster_size[1] = cluster_size_temp[1];
    
    
    cluster_sse[0] = getSSEOfCluster(0, dim, data, k, 
                    cluster_start, cluster_size,
                   cluster_centroid);
    cluster_sse[1] = getSSEOfCluster(1, dim, data, k, 
                    cluster_start, cluster_size,
                   cluster_centroid);
    ...
    // 1.5 Printing all the selected cluster centers.
        if(LOG) {
            printf("\nBefore Begin:\n");
            printf("Clusters after %d center points are : \n",j);
            for(int i=0;i<j;i++) {
                //printf("%d. %d -> ", i+1, cluster_centroid[i]);
                for(int p=0;p<dim;p++) {               
                    printf("%d. %0.4lf, ", i, cluster_centroid[i*dim+p]);
                }
                printf("\n");
            }
            printf("Cluster start, cluster_size is \n");
            for(int i=0;i<j;i++) {
    ...
    while( j < k){
        ...
        // find max see cluster
        int max_see_cluster_index = 0;
        int max_see_cluster_value = cluster_sse[0];
        for(int i=1;i<j;i++) {
            if(max_see_cluster_value < cluster_sse[i]) {
                max_see_cluster_value = cluster_sse[i];
                max_see_cluster_index = i;
            }
        }
        
        i0 = cluster_start[max_see_cluster_index];
        im = i0 + cluster_size[max_see_cluster_index]-1;       
        apply_2_means(dim, i0,im,data,2,cluster_start_temp,
                     cluster_size_temp,cluster_radius,cluster_centroid_temp,
                                        cluster_assign);
        
        if(LOG) printf("cluster_start_temp[0] %d , cluster_start_temp[1] %d ,cluster_size_temp[0] %d , cluster_size_temp[1] %d", cluster_start_temp[0],cluster_start_temp[1],cluster_size_temp[0],
               cluster_size_temp[1]);
        
        cluster_size[max_see_cluster_index] = cluster_size_temp[0];
        
        cluster_start[j] = cluster_start[max_see_cluster_index] +cluster_size_temp[0];
        cluster_size[j] = cluster_size_temp[1];
        
        for(int i = 0, t = max_see_cluster_index ; i < dim; i++,t++){
                cluster_centroid[t] = cluster_centroid_temp[i];                
        }
        for(int i = dim, t = (j)*dim; i < 2*dim; i++,t++){
                cluster_centroid[t] = cluster_centroid_temp[i];
        }
        cluster_sse[max_see_cluster_index] = getSSEOfCluster(max_see_cluster_index, 
                    dim, data, k, 
                    cluster_start, cluster_size,
                   cluster_centroid);
        cluster_sse[j] = getSSEOfCluster(j, dim, data, k, 
                    cluster_start, cluster_size,
                   cluster_centroid);
        ...
        // 1.5 Printing all the selected cluster centers.
        if(LOG) {
            printf("\nSelected Cluster is : %d\n",max_see_cluster_index);
            printf("Clusters after %d center points are : \n",j);
            for(int i=0;i<j;i++) {
                //printf("%d. %d -> ", i+1, cluster_centroid[i]);
                for(int p=0;p<dim;p++) {               
    ...
    if(LOG) {
        ...
        for(int i=0;i<k;i++) {
            printf("%d. %d -> ", i+1, cluster_centroid[i]);
            for(int j=0;j<dim;j++) {               
                printf("%lf, ", cluster_centroid[i*dim+j]);
            }
            printf("\n");
        }
    }
...
int kmeans(int dim, int i0, int im, double *data, int k, 
    ...
    int ndata = im-i0+1;
    if(LOG)  printf(" i0 = %d, im = %d,  ndata = %d\n",i0,im,ndata);
...
int main() {
    ...
    // 1. initial_centers
    k = initial_centers_bkm(dim, i0, im, data, k, 
                    cluster_centroid);
